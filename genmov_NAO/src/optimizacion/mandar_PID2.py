...
#             ankle = np.array(motion.getPosition("LLeg", 1, True)[0:3])
...
#             com = motion.getCOM("Body", 1, True)
#             ankle = np.array(motion.getPosition("RLeg", 1, True)[0:3])
#             support = ankle + np.array([0.015, 0.00, 0])
#             # pitch_index = 12
#             # roll_index = 13
...
#                 q_ankles[0] += 0*pitch_error#pid_pitch.compute(pitch_error, dt_pid)  # LAnklePitch
#                 q_ankles[1] += 0*roll_error#pid_roll.compute(roll_error, dt_pid)    # LAnkleRoll
#                 motion.setAngles(ankle_names[:2], q_ankles[:2], 0.5)
    ...
    if inicial:
        ...
        if current_support is not None:
            ...
            if current_support == "left":
                ...
                q_pid[12] += pid_pitch.compute(pitch_error, dt_frame)  # RAnklePitch
                q_pid[13] += pid_roll.compute(roll_error, dt_frame)    # RAnkleRoll
...
def main():
    ...
    app.start()
    session = app.session
    print(f"Successfully connected to the robot")
